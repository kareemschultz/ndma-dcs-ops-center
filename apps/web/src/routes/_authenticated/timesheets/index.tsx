...
function EntriesPanel({
  ...
  // STAGE 3 — data linking: approved leave + lateness overlapping this period.
  const { data: overlay } = useQuery(
    orpc.timesheets.periodOverlay.queryOptions({ input: { id: timesheet.id } }),
  );
  ...
  const leaveDays = overlay?.leaveDays ?? [];
  const lateness = overlay?.lateness ?? [];

  return (
    <div className="bg-muted/30 border-t px-4 py-3">
      {/* Linked context from other modules — read-only informational rows */}
      {(leaveDays.length > 0 || lateness.length > 0) && (
        <div className="mb-3 space-y-2">
          {leaveDays.length > 0 && (
            <div className="rounded-md border border-amber-200 bg-amber-50/60 px-3 py-2 dark:border-amber-900/40 dark:bg-amber-950/20">
              <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-amber-800 dark:text-amber-300">
                ...
                Approved Leave in this period ({leaveDays.length})
              </div>
              <div className="flex flex-wrap gap-1.5">
                {leaveDays.map((d) => (
                  <span
                    key={d.date}
                    className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-800 dark:bg-amber-900/30 dark:text-amber-300"
                  >
                    {format(parseISO(d.date), "dd MMM")} · {d.leaveType}
                  </span>
                ))}
              </div>
            </div>
          ...
          {lateness.length > 0 && (
            <div className="rounded-md border border-orange-200 bg-orange-50/60 px-3 py-2 dark:border-orange-900/40 dark:bg-orange-950/20">
              <div className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-orange-800 dark:text-orange-300">
                <Clock3 className="size-3.5" />
                Lateness recorded in this period
              </div>
              <div className="flex flex-wrap gap-1.5">
                {lateness.map((l) => (
                  <span
                    key={`${l.year}-${l.month}`}
                    className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-2 py-0.5 text-[11px] font-medium text-orange-800 dark:bg-orange-900/30 dark:text-orange-300"
                  >
                    {l.month} {l.year} · {l.daysLate} day{l.daysLate !== 1 ? "s" : ""} late ({l.totalTimeLate})
                  </span>
                ))}
              </div>
            </div>
