...
function LeavePage() {
  ...
  const visibleTypes = useMemo(
    () => sortLeaveTypesByCanonicalOrder((leaveTypes ?? []).filter(isVisibleLeaveType)),
    [leaveTypes],
  );
  ...
  const balanceRows = useMemo(() => {
    ...
    return visibleTypes.map((lt) => {
      ...
      const allowance = bal ? ((bal.entitlement ?? lt.annualEntitlement ?? 0) + (bal.carryOver ?? 0) + (bal.adjustment ?? 0)) : (lt.annualEntitlement ?? 0);
              ...
              <TableBody>
                {rows.length === 0 ? (
                  ...
                  <TableRow key={r.id}>
                    ...
                    <TableCell className="text-sm">{r.approver?.user?.name ?? "—"}</TableCell>
                    <TableCell>
                      {r.status === "pending" && (
                        <div className="flex gap-1">
                          ...
                          <Button size="icon" variant="ghost" className="size-7 text-red-500 hover:text-red-600"
                            onClick={() => rejectMutation.mutate({ id: r.id, reason: "" })}
