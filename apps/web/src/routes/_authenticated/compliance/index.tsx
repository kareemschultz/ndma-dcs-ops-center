...
function ComplianceHubPage() {
  // PPE matrix — orpc.ppe.issuances.matrix is the procedure (returns { items, rows })
  const { data: ppeMatrix, isLoading: ppeLoading } = useQuery(
  ...
  // Derive counts
  // ppeMatrix.rows = one row per staff member; ppeMatrix.items = active PPE items
  const ppeRows = ppeMatrix?.rows ?? [];
  const ppeItemCount = ppeMatrix?.items?.length ?? 0;
  // Total = number of (staff × ppe item) combinations
  const ppeTotal   = ppeRows.length * ppeItemCount;
  // Issues = issuances marked as lost, damaged, or not_issued
  const ppeIssues  = ppeRows.reduce((count, row) => {
    const issuances = Object.values(row.issuances as Record<string, { status?: string }> ?? {});
    return count + issuances.filter((i) => i.status === "lost" || i.status === "damaged" || i.status === "not_issued").length;
  }, 0);

  const trainingTotal  = (trainingCompliance as unknown[])?.length ?? 0;
  const trainingIssues = (trainingCompliance as Array<{ status?: string }>)?.filter((t) => t.status === "expired" || t.status === "expiring_soon").length ?? 0;

  const policyTotal  = (policyAck as unknown[])?.length ?? 0;
  const policyPending = (policyAck as Array<{ acknowledgedAt?: unknown }>)?.filter((p) => !p.acknowledgedAt).length ?? 0;
