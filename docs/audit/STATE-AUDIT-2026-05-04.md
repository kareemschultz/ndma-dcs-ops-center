...
| `02-dcs/appraisal-tracker/` | `APPRAISAL TRACKER DCS.xlsx` (63 rows + FeedbackFromStaff) | 4 | ✅ `appraisal_tracker_view` design pending verification (need to confirm DB VIEW exists) | ⛔ Phase 14 step 9 |
...
| `03-noc/performance-journal/` | `StaffPerformanceJournal_20230731_v01.xlsx` (12 staff × 4 years × 12 months × 4 categories); `StaffCommendationJournal_20231216_v01.xlsx` | 5 | 🟡 `performance_journal_entries` schema in `hr-docs.ts`; commendations table NOT YET in schema (master plan §5.3 specifies `commendations` table, no migration found) | ⛔ Phase 14 steps 10-11 (~2,304 journal + ~250 commendations) |
...
|---|---|---|---|
| §5.3 | `commendations` table | Phase 5 | ⛔ NOT FOUND in schema files. Phase 5 PR #26 shipped `noc_ticket_activity`, `noc_monthly_metrics`, `employee_of_the_month` — but commendations may be folded into `performance_journal_entries` or pending. **Verification needed.** |
| §5.3 | `appraisal_tracker_view` (DB VIEW) | Phase 4 | 🟡 Not found in schema search. Possibly created via raw SQL or pending. **Verification needed.** |
